...
import se.sics.gvod.bootstrap.server.BootstrapServerMemory;
...
import se.sics.gvod.simulator.vod.newp.BootstrapServerStub;
import se.sics.gvod.simulator.vod.newp.NewVodSimulator;
...
public final class VodSimulationMain extends ComponentDefinition {
    ...
    public VodSimulationMain() throws InterruptedException, IOException {
        ...
        Component bootstrapServer = create(BootstrapServerStub.class);
//        Component monitorServer = create(VodMonitorServer.class);
        Component gvodSimulator = create(NewVodSimulator.class);

        // loading component configurations
        final BootstrapConfiguration bootConfiguration
                = (BootstrapConfiguration) BootstrapConfiguration.load(BootstrapConfiguration.class);
...
//                = (GradientConfiguration) GradientConfiguration.load(GradientConfiguration.class);
        final VodConfiguration gvodConfiguration
                = (VodConfiguration) VodConfiguration.load(VodConfiguration.class);
...
//        connect(monitorServer.getNegative(Timer.class), p2pSimulator.getPositive(Timer.class));
        ...
        connect(gvodSimulator.getNegative(VodExperiment.class), p2pSimulator.getPositive(VodExperiment.class));
...
//        trigger(new VodSimulatorInit(bootConfiguration, monitorConfiguration, croupierConfiguration, gradientConfiguration, gvodConfiguration), gvodSimulator.getControl());
        trigger(new VodSimulatorInit(bootConfiguration, null, croupierConfiguration, null, gvodConfiguration), gvodSimulator.getControl());
